...
class ShootNumbers(models.Model):
    ...
    def generate(self):
        ...
        df = pd.DataFrame(list(DecidedNumbers.objects.all().values('shotDate','band')))  # qury set를 dataframe으로 변환
        df_band = df.sort_values(by='shotDate', ascending=False)  # 날짜를 기준으로 내림차순으로 정렬
        df_band = df_band.set_index('shotDate')

        print ('가장 최근 값:', df_band.iloc[0])
        band = 2
        if (band == 2):
            band_1st = 3
            band_2nd = 4
